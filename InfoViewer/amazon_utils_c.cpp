...
void amazon_SHA1_update(amazon_SHA1Context *context, const unsigned char *data, unsigned int len)
{
	...
	if ((j + len) > 63) {
		...
		amazon_SHA1_transform(context->state, context->buffer);
		for (; (i + 63) < len; i += 64) {
			amazon_SHA1_transform(context->state, &(data[i]));
		}
		j = 0;
	}
	else {
...
void amazon_SHA1_final(unsigned char digest[20], amazon_SHA1Context *context)
{
	...
	amazon_SHA1_update(context, (unsigned char *) "\200", 1);

	while ((context->count[0] & 504) != 448) {
		amazon_SHA1_update(context, (unsigned char *) "\0", 1);
	}

	amazon_SHA1_update(context, finalcount, 8);
...
void amazon_HMAC_SHA1(unsigned char hmac[20], const unsigned char *key, int key_len, const unsigned char *message, int message_len)
{
	...
	amazon_SHA1Context context;

	amazon_SHA1_init(&context);
	amazon_SHA1_update(&context, kipad, 64);
	amazon_SHA1_update(&context, message, message_len);
	amazon_SHA1_final(digest, &context);

	amazon_SHA1_init(&context);
	amazon_SHA1_update(&context, kopad, 64);
	amazon_SHA1_update(&context, digest, 20);
	amazon_SHA1_final(hmac, &context);
}
